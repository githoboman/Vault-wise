...
1. **Stacks Vault (`/bitcredit/stacks-vault`)**: Harmony-compatible Clarity smart contracts (Clarity 4) for locking sBTC collateral.
2. **Creditcoin USC (`/bitcredit/creditcoin-usc`)**: Solidity smart contracts for the Credit Power NFT, MockUSDC, and the Lending Pool.
3. **Relayer Node (`/bitcredit/relayer`)**: A high-performance Node.js service that monitors EVM events and Stacks block headers to automate cross-chain settlement.
4. **Dashboard Frontend (`/bitcredit/frontend`)**: A Next.js application designed with `next-themes` and Tailwind for a premium user experience.
...
cd bitcredit/stacks-vault && npx clarinet deploy --testnet
